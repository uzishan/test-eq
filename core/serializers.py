...
class BuildingSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def _get_avg_temperature(self, obj):
        ...
        temp = SensorData.objects.filter(sensor_room_floor_building__in='id').filter(sensor__type="temperature").aggregate(Avg('value'))
        return  temp

    def _get_avg_humidity(self, obj):
        # magia filtrului per buildingu asta.
        hum = SensorData.objects.filter(sensor_room_floor_building__in='id').filter(sensor__type="humidity").aggregate(Avg('value'))
        return hum
